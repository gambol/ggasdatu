...
<head>
<meta charset="utf-8" />
<title>< c:out value="${serverInfo.name}"/> , 新开<c:out value="${categoryName}" />私服, 阿土游戏,传奇私服,新开传奇私服,传奇私服发布网,魔兽私服,新开各种私服
</title>
			...
			<section class="width5">
				<div class="pageline">
					<form action="" method="get">
						<input type="hidden" name="categoryId" value="${serverInfo.categoryId}" /> <input class="search-button" type="submit" id="button" value="" name="search" /> 
				...
				<div>
					<div class="colgroup">
						<div style="padding: 10px" class="colgroup">
							<h3> < c:out value="${serverInfo.name}"/> (${categoryName}私服) </h3>
							<div class="width3 first" style="float: left">
								...
								<table class="no-style full">
										<tbody>
											<tr>
												<td><b>发布时间:</b></td>
												<td class="ta-right"><fmt:formatDate value="${serverInfo.publishTime}" pattern="yyyy年MM月dd日" /></td>
											</tr>
											<tr>
												<td><b>网站线路:</b></td>
												<td class="ta-right"><c:out value="${serverInfo.line}" /></td>
											</tr>
											<tr>
												<td><b>好评数目:</b></td>
												<td class="ta-right">${serverSysInfo.voteIn}</td>
											</tr>
							...
							<div class="width4 first ">
								<p>
									<c:out value="${serverInfo.description}" escapeXml="true" />
								</p>
							</div>
							<div class="width4 first" >
							<p class="box box-info">私服介绍</p>
							严格意义上的所谓sf是指一个没有得到网络游戏的制作商法定许可而私自存在幵运营的朋务器 它在技术和朋务实力上都和正式的官方朋务器存在可比性.因为这些技术和朋务的存在就是合法的。sf存在的主要目的同官方朋务器是一样的, 都是向玩家收费以获利。有些sf幵对外开放也以收费盈利为目的. 如部分网吧在吧内局域网架设sf. 这类sf规模较小,一般仅限于网吧内部。其目的也仅以娱乐为主.对玩家收费
							</div>
						</div>
			...
			<aside class="colgroup width2" id="asider">
				<div class="clean-content-box">
					<section class="notes-total">
						<img src="/img/atu.png" width="166px" height="42px" alt="阿土游戏" /> <a class="big-btn btn-yellow" href="/user/publish.htm">免费发布新站</a>
					</section>
				</div>
				<hr>
			</aside>
