...
public class PublishController {
	...
	public String form(HttpServletRequest request, HttpServletResponse response, Model model) throws ServletException,
			IOException {
		int userId = 0;
		int serverId = 0;
	...
	public String processSubmit(@Valid PublishBean publishBean, BindingResult result, HttpServletRequest request,
		...
		server.setName(publishBean.getServerName());
		...
		if (sid <= 0) {
			...
			if (server.getUserId() != userId) {
				logger.log(Level.WARNING, "server对应的userId 和 新的UserId对应不上. old userId:" + server.getUserId()
						+ " new UserId:" + userId);
